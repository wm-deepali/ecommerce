...
<div class="main-section">
    ...
    .sp-shipped{background:var(--purple-bg);color:var(--purple);}
    .sp-shipped::before{background:var(--purple);}
    ...
    <a href="{{ route('admin.reports.order-reports.export-excel', request()->query()) }}" class="btn-secondary-dash"><i class="fa fa-file-excel-o"></i> Export Excel</a>
    <a href="{{ route('admin.reports.order-reports.export-pdf', request()->query()) }}" class="btn-primary-dash"><i class="fa fa-download"></i> Export PDF</a>
</div>
            </div>
            ...
            <div class="date-bar">
                <div class="date-bar-left">
                    ...
                    <a href="{{ route('admin.reports.order-reports', ['preset' => 'today']) }}" class="date-preset {{ $preset === 'today' ? 'active' : '' }}">Today</a>
                    <a href="{{ route('admin.reports.order-reports', ['preset' => 'yesterday']) }}" class="date-preset {{ $preset === 'yesterday' ? 'active' : '' }}">Yesterday</a>
                    <a href="{{ route('admin.reports.order-reports', ['preset' => 'this_month']) }}" class="date-preset {{ $preset === 'this_month' ? 'active' : '' }}">This Month</a>
                    <a href="{{ route('admin.reports.order-reports', ['preset' => 'last_month']) }}" class="date-preset {{ $preset === 'last_month' ? 'active' : '' }}">Last Month</a>
                    <a href="{{ route('admin.reports.order-reports', ['preset' => 'this_year']) }}" class="date-preset {{ $preset === 'this_year' ? 'active' : '' }}">This Year</a>
                    <span class="date-preset {{ $preset === 'custom' ? 'active' : '' }}" id="customToggle" onclick="toggleCustom()">Custom</span>
                </div>
                <form method="GET" action="{{ route('admin.reports.order-reports') }}" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap" id="customInputs">
                    <input type="hidden" name="preset" value="custom">
                    <input type="date" class="date-input" name="start_date" value="{{ $from->format('Y-m-d') }}" id="startDate">
                    <span style="color:var(--text-hint);font-size:13px">→</span>
                    <input type="date" class="date-input" name="end_date" value="{{ $to->format('Y-m-d') }}" id="endDate">
            ...
            <div class="kpi-strip">
                <div class="kpi-tile">
                    ...
                    @if($ordersGrowth !== null)
                        <div class="kpi-badge {{ $ordersGrowth >= 0 ? 'up' : 'down' }}"><i class="fa fa-arrow-{{ $ordersGrowth >= 0 ? 'up' : 'down' }}"></i> {{ abs($ordersGrowth) }}% vs prev</div>
                    @else
                ...
                <div class="kpi-tile">
                    ...
                    @if($cancelledGrowth !== null)
                        <div class="kpi-badge {{ $cancelledGrowth <= 0 ? 'up' : 'down' }}"><i class="fa fa-arrow-{{ $cancelledGrowth >= 0 ? 'up' : 'down' }}"></i> {{ abs($cancelledGrowth) }}% vs prev</div>
                    @else
                        <div class="kpi-badge neutral">No prior data</div>
                    @endif
                </div>
            ...
            <div class="charts-2col">
                <div class="sc">
                    <div class="sc-head">
                        ...
                        <span class="sc-head-sub">Daily · {{ $from->format('d M Y') }} – {{ $to->format('d M Y') }}</span>
                    </div>
                    <div class="sc-body">
                        <div style="font-size:12px;color:var(--text-hint);margin-bottom:10px">
                            📦 Busiest day: <strong style="color:var(--text-primary)">{{ $busiestDay ?? '—' }}</strong> — {{ $busiestCount }} orders
                        </div>
                    ...
                    <div class="sc-body">
                        <div class="compare-strip">
                            <div class="compare-cell">
                                ...
                                <div class="compare-cell-sub">{{ $from->format('d M Y') }} – {{ $to->format('d M Y') }}</div>
                            </div>
                            <div class="compare-cell" style="background:#fafafa">
                                <div class="compare-cell-label">Last Period</div>
                                <div class="compare-cell-value" style="color:var(--text-secondary)">{{ number_format($firstHalfPrev + $secondHalfPrev) }}</div>
                            ...
                            @forelse($dailyBreakdown as $row)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span class="growth {{ $row['vs_yesterday'] > 0 ? 'up' : ($row['vs_yesterday'] < 0 ? 'down' : 'neutral') }}">
                                                <i class="fa fa-arrow-{{ $row['vs_yesterday'] >= 0 ? 'up' : 'down' }}"></i> {{ abs($row['vs_yesterday']) }}%
                                            </span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr><td colspan="5" style="text-align:center;padding:20px;color:var(--text-hint)">No data.</td></tr>
                            @endforelse
                            </tbody>
                            <tfoot>
                                <tr>
            ...
            <div class="sc" style="margin-bottom:20px">
                ...
                <div style="overflow-x:auto">
                    <table class="sum-table">
                        ...
                        @forelse($orders as $o)
                            <tr>
                                <td><a href="{{ route('admin.orders.show', $o->id) }}" style="font-size:12.5px;font-family:'SF Mono','Fira Code',monospace;color:var(--accent);text-decoration:none;font-weight:600">#{{ $o->order_number }}</a></td>
                                <td><div style="font-weight:560;font-size:13px">{{ $o->customer_name }}</div><div style="font-size:11.5px;color:var(--text-hint)">{{ $o->city?->name }}</div></td>
                                <td style="font-size:13px;color:var(--text-secondary)">{{ $o->created_at->format('d M Y') }}<br><span style="font-size:11.5px;color:var(--text-hint)">{{ $o->created_at->format('h:i A') }}</span></td>
                                <td class="units-cell">{{ $o->items->count() }}</td>
                                <td class="rev-cell">₹{{ number_format($o->grand_total, 0) }}</td>
                                <td><span style="font-size:12.5px;color:var(--text-secondary)">{{ strtoupper($o->payment_method) }}</span></td>
                                <td>
                                    <span style="font-size:12.5px;color:var(--text-secondary)">{{ $o->courier?->name ?? '—' }}</span>
                                    @if($o->tracking_number)
                                        <div style="font-size:11px;font-family:'SF Mono',monospace;color:var(--text-hint)">{{ $o->tracking_number }}</div>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if($o->status === 'delivered')
                                        ...
                                        @if($deliveredEntry)
                                            <span style="font-size:13px;font-weight:600;color:var(--green)">{{ $o->created_at->diffInDays($deliveredEntry->created_at) }}d</span>
                                        @else
                        ...
                        @empty
                            <tr><td colspan="9" style="text-align:center;padding:30px;color:var(--text-hint)">No orders found for this period.</td></tr>
                        @endforelse
                        </tbody>
                        <tfoot>
                            <tr>
                ...
                <div style="padding:14px 20px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
                    <span style="font-size:12.5px;color:var(--text-hint)">Showing {{ $orders->firstItem() ?? 0 }}–{{ $orders->lastItem() ?? 0 }} of {{ $orders->total() }} orders</span>
            ...
            <div class="charts-equal">
                    ...
                    <div style="overflow-x:auto">
                        <table class="sum-table">
                            ...
                            @forelse($cancelReasons as $reason)
                                ...
                                <tr>
                                    <td style="font-size:13px;font-weight:500">{{ $reason->remarks ?: 'No reason given' }}</td>
                                    <td class="units-cell">{{ $reason->c }}</td>
                                    <td><div style="display:flex;align-items:center;gap:8px"><div class="prog-bar"><div class="prog-fill" style="width:{{ $pct }}%;background:var(--red)"></div></div><span style="font-size:12px;color:var(--text-hint)">{{ $pct }}%</span></div></td>
                                    <td><span class="growth neutral">—</span></td>
                                </tr>
                            @empty
                                <tr><td colspan="4" style="text-align:center;padding:20px;color:var(--text-hint)">No cancellations in this period.</td></tr>
                            @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="sc">
                    ...
                    <div class="sc-body">
                        <div class="info-row"><span class="info-label">Total Orders</span><span class="info-value">{{ number_format($totalOrders) }}</span></div>
                        <div class="info-row"><span class="info-label">Delivered</span><span class="info-value" style="color:var(--green)">{{ $delivered }} ({{ $deliveryRate }}%)</span></div>
                        <div class="info-row"><span class="info-label">Processing / Shipped</span><span class="info-value" style="color:var(--blue)">{{ $processingShipped }}</span></div>
                        <div class="info-row"><span class="info-label">Pending</span><span class="info-value" style="color:var(--amber)">{{ $pending }}</span></div>
                        <div class="info-row"><span class="info-label">Cancelled</span><span class="info-value" style="color:var(--red)">{{ $cancelled }} ({{ $totalOrders > 0 ? round($cancelled / $totalOrders * 100, 1) : 0 }}%)</span></div>
                        <div class="info-row"><span class="info-label">Returned</span><span class="info-value" style="color:var(--purple)">{{ $returnedCount }} ({{ $returnRate }}%)</span></div>
                        <div class="info-row"><span class="info-label">Avg. Fulfilment Time</span><span class="info-value">{{ $avgFulfilment }} days</span></div>
                        <div class="info-row"><span class="info-label">Peak Order Hour</span><span class="info-value">{{ $peakHour ? \Carbon\Carbon::createFromTime($peakHour->h)->format('g A') . ' – ' . \Carbon\Carbon::createFromTime($peakHour->h)->addHour()->format('g A') : '—' }}</span></div>
                        <div class="info-row"><span class="info-label">Orders via COD</span><span class="info-value">{{ $codCount }} ({{ $totalOrders > 0 ? round($codCount / $totalOrders * 100) : 0 }}%)</span></div>
                        <div class="info-row" style="border-top:2px solid var(--border);margin-top:4px;padding-top:12px">
                            ...
                            <span style="font-size:18px;font-weight:750;color:var(--accent)">₹{{ number_format($avgOrderValue) }}</span>
                        </div>
                    </div>
                </div>
            </div>
